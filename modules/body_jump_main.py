...
class JumpingJackDetector(BaseDetector):
    ...
    def __init__(self, config=None, language='ZH_TW'):
        ...
        self.cfg = {
            'arm_down_angle': 45,
            'arm_up_angle': 155,
            'elbow_straight_angle': 120,
            'knee_straight_angle': 150,
    ...
    def process_frame(self, img, landmarks):
        ...
        # ========== 動作邏輯判斷 ==========
        # 判斷手臂放下
        if angle_12 <= self.arm_down_angle and angle_11 <= self.arm_down_angle:
            if not self.A:
                self.A = True
                self.B = False
        ...
        # 判斷手臂舉起
        if angle_12 >= self.arm_up_angle and angle_11 >= self.arm_up_angle:
            if self.A and not self.B:
                self.B = True
                self._resume_timer()  # 動作觸發，開始計時或重置閒置時間
        
        # 錯誤判斷
        if angle_13 <= self.elbow_straight_angle or angle_14 <= self.elbow_straight_angle:
            self.arm_wrong = True
        ...
        if (self.A and self.B) and \
           angle_12 <= self.arm_down_angle and angle_11 <= self.arm_down_angle and \
           angle_26 >= self.knee_strict_angle and angle_25 >= self.knee_strict_angle:
            ...
            if self.arm_wrong or self.knee_wrong:
                ...
                if self.arm_wrong:
                    print(self.lang.log('error', 'fail_elbow_bent'))
                    cv2.circle(img, (int(b[0]), int(b[1])), 10, (0, 0, 255), -1)
                    cv2.circle(img, (int(b2[0]), int(b2[1])), 10, (0, 0, 255), -1)
